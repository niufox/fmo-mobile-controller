...
import { ModelDownloadManager } from '../../../model-download-manager.js';
...
export class FighterRenderer extends BaseRenderer {
    ...
    /**
     * Load GLB Model with Progress Bar (on-demand)
     */
    loadModel() {
        ...
        console.log('[FighterRenderer] Loading model from:', modelUrl);
        ...
        const remainingEl = document.getElementById('loader-remaining');
        ...
        loader.load(
            ...
            (xhr) => {
                ...
                if (xhr.total > 0) {
                    ...
                    const speed = xhr.loaded / (xhr.timeStamp / 1000 || 1);
                    const speedStr = this.downloadManager.formatBytes(speed) + '/s';
                    if (speedEl) speedEl.textContent = speedStr;

                    // Calculate remaining time
                    const remaining = (xhr.total - xhr.loaded) / speed;
                    const remainingStr = this.downloadManager.formatTime(remaining);
                    if (remainingEl) remainingEl.textContent = remainingStr;
                }
            },
            (error) => {
                // Error
                ...
                // Hide download info
                if (loaderEl) {
                    const loaderInfo = loaderEl.querySelector('.loader-info');
    ...
    createFallbackModel() {
        ...
        // Create a simple fighter shape using basic geometry
        ...
        this.baseModel.add(cockpit);

        // Engine glow
        const engineGeometry = new THREE.CylinderGeometry(0.2, 0.1, 0.5, 16);
        ...
        this.baseModel.add(engine);

        this.modelLoaded = true;

        // Re-create formation with fallback model
        this.updateFormationModels();
    }
    ...
    /**
     * Public method to trigger model loading on-demand
     */
    ensureModelLoaded() {
