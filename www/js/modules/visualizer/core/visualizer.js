...
export class Visualizer {
    ...
    switchMode() {
        ...
        // Load model on-demand when switching to FIGHTER mode (mode 6)
        if (this.mode === 6) {
            const fighterRenderer = this.renderers[6];
            if (fighterRenderer && typeof fighterRenderer.ensureModelLoaded === 'function') {
                fighterRenderer.ensureModelLoaded();
            }
        }
