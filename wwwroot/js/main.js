// Note: api3.0 modules available but not used in current implementation
// This file uses its own WebSocket implementation (ControlClient class)
// import { wsService, WebSocketService } from './api3.0/webSocketService.js';
// import { AudioStreamPlayer, initAudioUI } from './api3.0/audioPlayer.js';
// import { RemoteService, getRemoteService } from './api3.0/remoteService.js';
// import { QsoService, getQsoService } from './api3.0/qsoService.js';
// import { ConfigService } from './api3.0/configService.js';
// import { EventService } from './api3.0/eventsService.js';
        ...
        class ControlClient extends EventEmitter {
            ...
            connect(host) {
                ...
                try {
                    ...
                    this.ws.onopen = () => {
                        ...
                        console.log('[ControlClient] /ws connected, delaying requests...');

                        // 延迟200ms后发送初始请求，避免和/audio、/events连接冲突
                        setTimeout(() => {
                            console.log('[ControlClient] Sending initial requests...');
                            this.fetchList(); // 初始加载
             ...
             send(type, subType, data = {}) {
                console.log('[ControlClient] Sending message:', { type, subType, data });
                if (!this.connected || !this.ws) {
                    console.warn('[ControlClient] Cannot send: not connected');
                    return;
                }
            ...
            handleMessage(msg) {
                // 路由不同类型的消息
                console.log('[ControlClient] Received message:', msg.type, msg.subType);

                if (msg.type === 'station') {
                    switch (msg.subType) {
                        case 'getListResponse':
                            console.log('[ControlClient] getListResponse received:', msg.data);
                            const newList = msg.data.list || [];
                            const start = msg.data.start;

                            console.log('[ControlClient] Station list data:', {
                                newListLength: newList.length,
                                start: start,
                                count: msg.data.count,
                                totalListLength: this.tempStationList.length
                            });

                            // 更新总台站数
                            if (msg.data.count !== undefined) {
                                this.totalStationCount = msg.data.count;
                            } else {
                                this.totalStationCount = msg.data.list.length;
                            }
                        ...
                        case 'setCurrentResponse':
                            ...
                            break;
                        default:
                            console.warn('[ControlClient] Unhandled station subType:', msg.subType, msg.data);
                    }
            ...
            // 全量获取逻辑 (递归/分片获取)
            fetchList() {
                console.log('[ControlClient] fetchList() called');
                console.log('[ControlClient] Current state:', {
                    connected: this.connected,
                    fetchingAll: this.fetchingAll,
                    fetchStart: this.fetchStart,
                    currentListLength: this.stationList.length
                });

                // 如果已经在全量加载，跳过
                if (this.fetchingAll) {
                    console.log('[ControlClient] Already fetching all, skipping');
                    return;
                }
                ...
                // 发起第一次请求
                console.log('[ControlClient] Sending getListRange request:', { start: 0, count: this.fetchPageSize });
        ...
        class AudioPlayer extends EventEmitter {
            ...
            // 静态连接锁 - 确保全局只有一个audio WebSocket连接
            static connectionLock = false;
            static currentWs = null;
            ...
            constructor({ url = `ws://${window.location.host}/audio`, inputSampleRate = 8000 } = {}) {
                // 如果已存在实例，返回该实例
                if (AudioPlayer.instance && AudioPlayer.instance !== this) {
                    return AudioPlayer.instance;
                }
            ...
            connect(host) {
                // 检查全局连接锁
                if (AudioPlayer.connectionLock) {
                    console.warn('[AudioPlayer] Connection in progress, skipping');
                    return;
                }

                // 如果已有连接，先断开
                if (AudioPlayer.currentWs) {
                    console.log('[AudioPlayer] Closing existing audio connection...');
                    try {
                        AudioPlayer.currentWs.close();
                    } catch (e) {}
                    AudioPlayer.currentWs = null;
                    this.ws = null;
                    this.connected = false;
                }

                // 设置连接锁
                AudioPlayer.connectionLock = true;

                if (this.ws && (this.connected || this.ws.readyState === WebSocket.CONNECTING)) {
                    AudioPlayer.connectionLock = false;
                    return;
                }
                ...
                console.log(`[AudioPlayer] Connecting to ${this.url}...`);

                this.ws = new WebSocket(this.url);
                AudioPlayer.currentWs = this.ws;
                ...
                this.ws.onopen = () => {
                    ...
                    console.log('[AudioPlayer] Connected successfully');
                    // Resume audio if it was suspended
                    if (this.audioCtx?.state === 'suspended') this.audioCtx.resume();
                };

                this.ws.onclose = (e) => {
                    console.log(`[AudioPlayer] Connection closed: ${e.code} - ${e.reason}`);
                    this.connected = false;
                    AudioPlayer.connectionLock = false;
                    // 只清除当前实例的引用
                    if (this.ws === AudioPlayer.currentWs) {
                        AudioPlayer.currentWs = null;
                    }
                    this.ws = null;
                    this.emit('status', false);
                ...
                this.ws.onerror = (e) => {
                    console.error('[AudioPlayer] Connection error:', e);
                    AudioPlayer.connectionLock = false;
                    this.emit('status', false);
            ...
            disconnect() {
                console.log('[AudioPlayer] Disconnecting...');

                // 关闭WebSocket
                if (this.ws) {
                    try { this.ws.close(); } catch (e) { console.error('[AudioPlayer] Close error:', e); }
                    // 如果这是当前全局连接，清除引用
                    if (this.ws === AudioPlayer.currentWs) {
                        AudioPlayer.currentWs = null;
                    }
            ...
            // 静态方法：强制断开所有audio连接（用于多标签页场景）
            static forceDisconnectAll() {
                if (AudioPlayer.currentWs) {
                    console.log('[AudioPlayer] Force disconnecting global audio connection...');
                    try {
                        AudioPlayer.currentWs.close();
                    } catch (e) { console.error('[AudioPlayer] Force close error:', e); }
                    AudioPlayer.currentWs = null;
                }
                AudioPlayer.connectionLock = false;
                if (AudioPlayer.instance) {
                    AudioPlayer.instance.ws = null;
                    AudioPlayer.instance.connected = false;
                }
            }
            ...
            static getConnectionInfo() {
                return {
                    hasInstance: AudioPlayer.instance !== null,
                    connected: AudioPlayer.instance ? AudioPlayer.instance.connected : false,
                    hasWs: AudioPlayer.currentWs !== null,
                    wsState: AudioPlayer.currentWs ? AudioPlayer.currentWs.readyState : null,
                    locked: AudioPlayer.connectionLock
                };
            }
            ...
            connect(host) {
                ...
                try {
                    ...
                    this.ws.onopen = () => {
                        this.connected = true;
                        this.retryMs = 1000;
                        console.log('[EventsClient] /events connected');
                        // 收到events连接成功，判定为已连接
                        this.emit('status', true);
                    };
                    ...
                    this.ws.onmessage = (e) => {
                        try {
                            ...
                            this.handleMessage(msg);
                            // 收到events信息后，自动隐藏设置框
                            if (typeof ui !== 'undefined' && ui && ui.settingsArea) {
                                ui.settingsArea.classList.remove('open');
                            }
                        } catch (err) {
        ...
        // --- DOMContentLoaded事件：确保DOM完全加载后再初始化 ---
        document.addEventListener('DOMContentLoaded', () => {
            console.log('[Init] DOM loaded, initializing application...');

            // --- 页面可见性监听：确保同一时间只有一个audio连接 ---
            document.addEventListener('visibilitychange', () => {
                if (document.hidden) {
                    // 页面隐藏时，断开audio连接以允许其他标签页使用
                    console.log('[PageVisibility] Page hidden, disconnecting audio...');
                    player.disconnect();
                } else {
                    // 页面显示时，如果之前已连接，重新连接
                    console.log('[PageVisibility] Page visible, checking connection...');
                    const connInfo = AudioPlayer.getConnectionInfo();
                    console.log('[PageVisibility] Connection info:', connInfo);
                }
            });

            // --- 页面卸载前强制断开audio连接 ---
            window.addEventListener('beforeunload', () => {
                console.log('[PageUnload] Force disconnecting audio connection...');
                AudioPlayer.forceDisconnectAll();
            });

        // --- 网络状态监听 ---
        window.addEventListener('online', () => {
            console.log('[Network] Online');
            if (typeof ctrl !== 'undefined' && ctrl && !ctrl.connected && typeof ui !== 'undefined' && ui && ui.inpHost) {
        ...
        events.on('status', (connected) => {
            console.log('[EventsClient] Status changed:', connected);
            // 收到events连接后，判定为连接成功
        ...
        ui.btnRecord.addEventListener('click', () => {
          ...
          // 4. 台站列表逻辑 - 性能优化版本
           ctrl.on('stationList', (list) => {
              console.log('[StationListUI] Rendering station list:', {
                  listLength: list.length,
                  totalCount: ctrl.totalStationCount
              });

               ui.stCount.textContent = `${list.length}/${ctrl.totalStationCount}`;
              ...
              if (ui.stCountModal) {
                  ui.stCountModal.textContent = `${list.length}/${ctrl.totalStationCount}`;
              }
